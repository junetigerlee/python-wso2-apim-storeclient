...
class DocumentCollectionApi(object):
    """
    ...
    def apis_api_id_documents_get(self, api_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.apis_api_id_documents_get(api_id, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.apis_api_id_documents_get_with_http_info(api_id, **kwargs)
        else:
            (data) = self.apis_api_id_documents_get_with_http_info(api_id, **kwargs)
            return data

    def apis_api_id_documents_get_with_http_info(self, api_id, **kwargs):
        """
        Get a list of documents of an API 
        This operation can be used to retrive a list of documents belonging to an API by providing the id of the API.  `X-WSO2-Tenant` header can be used to retrive documents of an API that belongs to a different tenant domain. If not specified super tenant will be used. If Authorization header is present in the request, the user's tenant associated with the access token will be used.  **NOTE:** * This operation does not require an Authorization header by default. But in order to see a restricted API's documents, you need to provide Authorization header. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.apis_api_id_documents_get_with_http_info(api_id, callback=callback_function)
        ...
        all_params = ['api_id', 'limit', 'offset', 'x_wso2_tenant', 'accept', 'if_none_match']
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method apis_api_id_documents_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'api_id' is set
        if ('api_id' not in params) or (params['api_id'] is None):
            raise ValueError("Missing the required parameter `api_id` when calling `apis_api_id_documents_get`")
        ...
        return self.api_client.call_api('/apis/{apiId}/documents', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
